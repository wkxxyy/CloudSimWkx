	...
	public boolean allocateHostForVm(Vm vm) {
		...
		if (!getVmTable().containsKey(vm.getUid())) { // if this vm was not created
			do {// we still trying until we find a host or until we try all of them
				...
				result = host.vmCreate(vm);//预处理一下，看vm是否合适在这个host

				if (result) { // if vm were succesfully created in the host
					getVmTable().put(vm.getUid(), host);//记录host当中到底运行了那些虚拟机
					getUsedPes().put(vm.getUid(), requiredPes);//修改已经使用的pe
					getFreePes().set(idx, getFreePes().get(idx) - requiredPes);//设置空闲的pe
					result = true;
					break;//如果当前的满足直接就结束了，所以就是先来先服务，先来满足就服务
				} else {
