...
public class DatacenterBroker extends SimEntity {
	...
	protected void processResourceCharacteristics(SimEvent ev) {
		...
		if (getDatacenterCharacteristicsList().size() == getDatacenterIdsList().size()) {
			setDatacenterRequestedIdsList(new ArrayList<Integer>());
			createVmsInDatacenter(getDatacenterIdsList().get(0));//开始发送创建broker中的vmlist列表中的vm
		}
	}
	...
	protected void processVmCreate(SimEvent ev) {
		...
		if (result == CloudSimTags.TRUE) {//如果这个返回的ack标识创建vm成功了在center中，那么更新一些属性
		...
		incrementVmsAcks();//ack请求加一

		// all the requested VMs have been created
		if (getVmsCreatedList().size() == getVmList().size() - getVmsDestroyed()) {
			submitCloudlets();//该提交任务了
		} else {
	...
	protected void createVmsInDatacenter(int datacenterId) {
		...
		for (Vm vm : getVmList()) {
			if (!getVmsToDatacentersMap().containsKey(vm.getId())) {//如果我还没创建虚拟机就继续
	...
	protected void submitCloudlets() {
		...
		for (Cloudlet printCloudlet:sortList){
			Log.printLine(count+".Cloudlet ID: "+printCloudlet.getCloudletId()+",CLoudlet Length:"+printCloudlet.getCloudLength());
			count++;
		}
		...
		for (Cloudlet cloudlet : sortList) {
			...
			if (cloudlet.getVmId() == -1) {//通过任务的vmid找到对应的vm
			...
			sendNow(getVmsToDatacentersMap().get(vm.getId())/*获取detacenter*/, CloudSimTags.CLOUDLET_SUBMIT, cloudlet);
			cloudletsSubmitted++;
