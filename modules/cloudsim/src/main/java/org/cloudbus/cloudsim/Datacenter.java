...
public class Datacenter extends SimEntity {
	...
	/** The vm provisioner. *///虚拟机分配策略
	private VmAllocationPolicy vmAllocationPolicy;

	/** The last time some cloudlet was processed in the datacenter. *///上一次处理时间
	...
	/** The scheduling delay to process each datacenter received event. *///调度间隔
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			case CloudSimTags.VM_DATACENTER_EVENT://定时器发来的，里面就是具体运行
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		boolean result = getVmAllocationPolicy().allocateHostForVm(vm);//创建虚拟机的时候先获取一种策略
		//例子1当前这个策略是之前创建center中设置的那个simple策略
		if (ack) {
			int[] data = new int[3];
			data[0] = getId();
			data[1] = vm.getId();
			...
			//发送消息到队列，告诉broker创建是否成功了
		}

		if (result) {//更新时间，
			getVmList().add(vm);

			if (vm.isBeingInstantiated()) {//vm是否被初始化
				vm.setBeingInstantiated(false);//初始化完了
			}
			//更新时间，运行时间
			vm.updateVmProcessing(CloudSim.clock(), getVmAllocationPolicy().getHost(vm).getVmScheduler()//因为在center创建的时候把host嵌套在了分配策略中，所以需要取出vm就需要取策略-取host-取vm
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		updateCloudletProcessing();//更新任务时间
		...
		try {
			...
			cl.setResourceParameter(//设置资源参数
			...
			Host host = getVmAllocationPolicy().getHost(vmId, userId);//确定vm所在的host，也就是任务所在的id
			Vm vm = host.getVm(vmId, userId);//获取vm所在的host
			CloudletScheduler scheduler = vm.getCloudletScheduler();//把任务绑定到了vm
			double estimatedFinishTime = scheduler.cloudletSubmit(cl, fileTransferTime);//评估任务完成时间

			// if this cloudlet is in the exec queue
			if (estimatedFinishTime > 0.0 && !Double.isInfinite(estimatedFinishTime)) {//是否能在有限时间内完成并且任务不为空
				estimatedFinishTime += fileTransferTime;
				send(getId(), estimatedFinishTime, CloudSimTags.VM_DATACENTER_EVENT);//给自己发一个定时器，到时间检查是否有完成
			}
